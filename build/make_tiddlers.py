...
IMGSRC = re.compile(r"!\[([^\]]*)\]\((?:\.\./)?((?:gold/)?crops/[\w.-]+\.jpg)\)")
...
def main():
    ...
    for p in sorted(WIKI.rglob("*.md")):
        ...
        body = IMGSRC.sub(
            lambda m: f'<$image source="{m.group(2)}" tooltip="{m.group(1)}" class="pga-scan"/>',
            body)
    ...
    # The verbatim source extractions are NOT bundled. Two reasons: they are
    # third-party copyrighted text (an AMS chapter, a Springer volume) and this
    # wiki is published publicly; and TiddlyWiki renders their "![](...)" lines
    ...
    # one tiddler per formula; pages transclude these (see prerender_math.cjs)
    ...
    crops = sorted(list((WIKI / "gold" / "crops").glob("*.jpg"))
                   + list((WIKI / "crops").glob("*.jpg")))
    for c in crops:
        write_tid(c.relative_to(WIKI).as_posix(), base64.b64encode(c.read_bytes()).decode(),
                  "crop", ttype="image/jpeg")
    if crops:
